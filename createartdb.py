...
class ArtDatabaseCreator:
    ...
    def __init__(self, csv_path: Optional[str] = None, db_path: Optional[str] = None) -> None:
        ...
        self.csv_path: Path = Path(csv_path) if csv_path else Path(config.get(ConfigConstants.ART_RECORDS_CSV, 'artrecords.csv'))
        self.db_path: Path = Path(db_path) if db_path else Path(config.get(ConfigConstants.ART_DATABASE, 'art.db'))
    ...
    def _load_field_types(self) -> Dict[str, str]:
        ...
        types_path: Path = Path(__file__).parent / 'artrecords.types'
        try:
    ...
    def _generate_embedding(self, row: Dict[str, str]) -> bytes:
        ...
        text_to_embed: str = ' '.join(str(row.get(col, '')) for col in self.embeddable_columns)
    ...
    def create_hnsw_index(self, no_of_records: int) -> None:
        ...
        try:
            ...
            M: int
            ef_construction: int
            M, ef_construction, _ = recommend_hnsw_params(no_of_records, self.vector_embeddings_dimensions)

            # Initialize HNSW index
            p: hnswlib.Index = hnswlib.Index(space='cosine', dim=self.vector_embeddings_dimensions)
            ...
            # Save the index
            index_path: Path = self.db_path.with_suffix('.hnsw')
